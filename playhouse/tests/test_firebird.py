...
firebird_db = database_initializer.get_database(
    'sqlite',
...
firebird_db.register_aggregate(WeightedAverage, 'weighted_avg', 1)
firebird_db.register_aggregate(WeightedAverage, 'weighted_avg2', 2)
firebird_db.register_collation(collate_reverse)
firebird_db.register_function(title_case)
...
class FirebirdTestCase(PeeweeTestCase):
    messages = [
    ...
    def test_granular_transaction(self):
        conn = firebird_db.get_conn()

        def test_locked_dbw(isolation_level):
            with firebird_db.granular_transaction(isolation_level):
                Post.create(message='p1')  # Will not be saved.
                conn2 = firebird_db._connect(firebird_db.database, **firebird_db.connect_kwargs)
        ...
        firebird_db.set_autocommit(False)
        ...
        conn.rollback()
        firebird_db.set_autocommit(True)

        with firebird_db.granular_transaction('deferred'):
...
class TestTransitiveClosureIntegration(PeeweeTestCase):
    ...
    def setUp(self):
        ...
        firebird_db.load_extension(CLOSURE_EXTENSION.rstrip('.so'))
        firebird_db.close()

    def tearDown(self):
        super(TestTransitiveClosureIntegration, self).tearDown()
        firebird_db.unload_extension(CLOSURE_EXTENSION.rstrip('.so'))
    ...
    def initialize_models(self):
        ...
        Closure = ClosureTable(Category)
        firebird_db.drop_tables([Category, Closure], True)
        firebird_db.create_tables([Category, Closure])
